<template>
  <div class="shopbag">
    ...
    <template #title>
      ...
      <template #right>
        <div class="right" v-if="shopbagData.length > 0">
          <!-- {{isEdit ? '完成' : '编辑'}} -->
          <img :src="isEdit ? wc : bj" alt="" class="auto-img">
        </div>
      </template>
    </van-nav-bar>
    <div v-if="shopbagData.length == 0">
      <van-empty description="购物袋空空如也，去逛一逛!">
        <van-button round color="#f0134b" class="bottom-button" @click="shopbag()">
          逛一逛
        </van-button>
      </van-empty>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      //编辑
      bj:require("../assets/images/bji.png"),
      //完成
